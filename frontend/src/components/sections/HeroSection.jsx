...
export default function HeroSection() {
  ...
  // Noovolife-style vertical parallax
  const { scrollYProgress } = useScroll({
    target: sectionRef,
    offset: ["start start", "end start"]
  });

  // text layer moves down (slower scroll)
  const textY = useTransform(scrollYProgress, [0, 1], ["0%", "50%"]);
  // blobs move up (reverse parallax)
  const blobsY = useTransform(scrollYProgress, [0, 1], ["0%", "-30%"]);
  // fade out content
  ...
  return (
    <section ref={sectionRef} id="home" className="relative pt-32 pb-20 md:pt-48 md:pb-32 overflow-hidden flex items-center justify-center min-h-[100vh]">
      ...
      <div className="container mx-auto px-4 md:px-6 relative z-10 h-full flex items-center">
        ...
        >
          ...
          <div className="overflow-hidden pb-4 px-2">
            <motion.h1
              variants={itemVariants}
              className="text-5xl md:text-7xl font-extrabold tracking-tight mb-2 bg-clip-text text-transparent bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 dark:from-blue-400 dark:via-indigo-400 dark:to-purple-400"
            >
          ...
          <motion.p
            variants={itemVariants}
            className="text-lg md:text-xl text-slate-600 dark:text-zinc-400 mb-10 max-w-2xl mt-4"
          >
